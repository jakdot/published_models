...
1. The file materials_final.csv are cleaned materials from the GECO corpus (just materials, no eye-tracking measures appear there)

2. The file summed_eyetracking_parameter_search.csv is the file used for paramater searching. It includes relevant eye-tracking measures.

3. The file summed_eyetracking_not_search.csv is the file used for checking predictions of the Basic ACT-R Reader model. It also includes relevant eye-tracking measures.
...
checkfound_MPI.py: This simulates reading on the data set summed_eyetracking_not_search.csv. It uses parameter values found independently using parametersearch_MPI.py. The values manually inserted come from the simulation done in the paper ``Building an ACT-R reader for eye-tracking corpus data''.
